      ...
      const handleResize = () => {
        if (typeof window !== 'undefined') {
          if (window.innerWidth < 640) {
            setScale(0.5); // Mobile screens
          } else if (window.innerWidth < 1024) {
            setScale(0.75); // Tablet screens
          } else {
            setScale(1); // Desktop
          }
        }
          ...
          {items.map((item, i) => {
            ...
            return (
              <div
                ...
                style={{
                  transform: `rotateY(${itemAngle}deg) translateZ(${radius * scale}px) scale(${scale})`,
                  left: '50%',
                  top: '50%',
                  marginLeft: '-150px',
                  marginTop: '-200px',
              ...
              >
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black/80 via-black/40 to-transparent text-white">
                    <h2 className="text-2xl font-black uppercase tracking-tighter">{item.common}</h2>
                    <em className="text-sm italic opacity-80 block mb-2">{item.binomial}</em>
                    <p className="text-xs opacity-50">Photo by: {item.photo.by}</p>
                  </div>
                </div>
              </div>
            );
