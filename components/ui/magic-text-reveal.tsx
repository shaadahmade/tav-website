  ...
  useEffect(() => {
    const handleResize = () => {
      if (wrapperRef.current && textDimensions.width && textDimensions.height) {
        // Responsive padding based on screen size and font size - made tighter
        const isMobile = window.innerWidth < 768;
        const basePadding = isMobile ? Math.max(fontSize * 0.1, 5) : Math.max(fontSize * 0.15, 10);
        
        const minWidth = textDimensions.width + basePadding * 2;
        const minHeight = textDimensions.height + basePadding * 2;
